...
func (f *SogouBak) unmarshalV2(r *bytes.Reader) []*Entry {
	...
	for i := 0; i < len(d)/2; i++ {
		...
		di = append(di, &Entry{word, pys, int(wordInfo.freq)})
		// fmt.Printf("word: %v\tcode: %v\tfreq: %v\n", word, codes, wordInfo.freq)
	}
	return di
}

func (f *SogouBak) decryptPinyin(r *bytes.Reader, offset uint32) []string {
	r.Seek(int64(offset), 0)
	n := ReadUint16(r) / 2
	ps := make([]string, 0)
	for i := _u16; i < n; i++ {
		p := ReadUint16(r)
		ps = append(ps, f.pyList[p])
	}
	return ps
}
...
func decryptWordsEx(r *bytes.Reader, offset uint32, p1, p2, p3 int) string {
	...
	ret := util.DecodeMust(decWords, "UTF-16LE")
	return ret
}
