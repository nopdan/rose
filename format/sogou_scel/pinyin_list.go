...
func list2bytes(list []string) []byte {
	b := make([]byte, 0, len(list)*2)
	b = append(b, util.To4Bytes(len(list))...)
	for i, s := range list {
		b = append(b, util.To2Bytes(i)...)
		py := utf16.Encode(s)
		b = append(b, util.To2Bytes(len(py))...)
		b = append(b, py...)
	}
	return b
}

func index(py string) int {
	// 拼音
	if idx, ok := syllMap[py]; ok {
		return idx
	}
	// 特例处理
	if len(py) == 1 && py == "#" {
...
func indexPinyin(idx int, pys []string) string {
	if idx < len(pys) {
		return pys[idx]
	} else if idx == 482 {
		return "#" // 特例，#号
	} else if idx-len(pys) > 10 {
		fmt.Printf("拼音索引 %d 超出范围\n", idx)
	}
	return strconv.Itoa(idx - len(pys))
}

// 默认的拼音表（适用于一些不带拼音表的词库）
var pyList = []string{
